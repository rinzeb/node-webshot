  ...
  */
  ...
  var spawn = function() {
    if (options.siteType === 'html') {
      var obj = tmp.fileSync({dir: "./public", postfix: ".html"});
      var tmpPath = obj.name;
      fs.writeSync(obj.fd, site, null, 'utf-8');
      fs.close(obj.fd);
      options.siteType = 'url';
      options.tempFile = tmpPath;
      site = 'http://localhost:3002/' + tmpPath.split('\\').pop();
  ...
  if (options.timeout) {
    timeoutID = setTimeout(function() {
      ...
      if (!calledCallback) {
        ...
        if (options.tempFile) {
          fs.exists(options.tempFile, function (exists) {
            if (exists) {
              fs.unlink(options.tempFile, function (err) { });
            }
          });
        }
